...
class ReconciliationServiceTest {
    ...
    void 드리프트_없으면_아무것도_보정하지_않는다() {
        ...
        when(preemption.preemptedAtMillisAll(SCHEDULE_ID)).thenReturn(Map.of());

        DriftReport report = service.reconcileSchedule(SCHEDULE_ID);

        verify(preemption, never()).removeSeat(anyLong(), anyLong());
        verify(preemption, never()).returnSeats(anyLong(), any());
    ...
    void missing_좌석이_선점흔적_없으면_풀로_되돌린다() {
        ...
        when(preemption.preemptedAtMillisAll(SCHEDULE_ID)).thenReturn(Map.of()); // 선점 기록 없음 = in-flight 아님

        DriftReport report = service.reconcileSchedule(SCHEDULE_ID);

        verify(preemption).returnSeats(SCHEDULE_ID, List.of(5L));
    ...
    void missing_좌석이_grace_밖_선점이면_풀로_되돌린다() {
        ...
        when(seatInventoryRepository.findAvailableIdsByScheduleId(SCHEDULE_ID)).thenReturn(List.of(5L));
        when(preemption.availableSeatIds(SCHEDULE_ID)).thenReturn(Set.of());
        when(preemption.preemptedAtMillisAll(SCHEDULE_ID)).thenReturn(Map.of(5L, staleTs));
    ...
    void missing_좌석이라도_grace_이내_최근_선점이면_되돌리지_않는다_오버셀_방지() {
        ...
        when(seatInventoryRepository.findAvailableIdsByScheduleId(SCHEDULE_ID)).thenReturn(List.of(5L));
        when(preemption.availableSeatIds(SCHEDULE_ID)).thenReturn(Set.of());
        when(preemption.preemptedAtMillisAll(SCHEDULE_ID)).thenReturn(Map.of(5L, recentTs));
