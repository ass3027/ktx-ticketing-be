...
> 상태: **확정·구현 완료(2026-06-09)** — §10 열린 결정 둘 확정(갈래 1 / active 분리), 구현은 `results/P3_Result.md` T3-10 참조
...
1. **SADD 안전화 기본 전략**: 갈래 1(좌석별 Redis 타임스탬프, 온라인 양방향) vs
   ...
   - **→ 확정: 갈래 1.** 증명/트레이드오프 서술이 평가 산출물이고 online 자가치유가 가치. 선점을 Lua 로
     원자화해 `HSET preempt:ts` 동반(선점 성공 시에만). 핫패스 HSET 1회 상시비용은 측정(P4)으로 확인.
2. **active:{id} 카운터 reconcile 범위**: T3-10에 포함 vs 별도 follow-up.
   - SoT가 DB 아닌 entry token 집합이라 성격이 다름. 분리 시 T3-10은 avail↔DB에 집중(범위 명확).
   - **→ 확정: 분리.** T3-10 은 avail↔DB 수렴에 집중. active 카운터 드리프트(버려진 세션)는 별도 follow-up.
