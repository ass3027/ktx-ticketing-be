...
- [x] **T6-6** 셀프 체크리스트 7항목 점검 **완료**(2026-07-14). C1~C7 산출물 대조 → **6/7 충족(C1·C2·C4·C5·C6·C7), C3만 부분 미달**(배포 URL·영상 대기 — T6-3b·T6-4/5 로 해소되는 산출물 공백이지 설계 결함 아님). 판정 근거는 아래 §제출 전 셀프 체크리스트에 항목별 기재. 변별력=C4(AI 게이트 워크플로우 실사례)·C7(E1 반직관적 결과 정직 서술)이 강점.
- [ ] **T6-7** 최종 점검 + 제출
...
- [~] **C3** 배포 URL + GitHub + README + 동작 영상 구비 — GitHub·README ✅, **배포 URL(T6-3b)·영상(T6-4/5) 미완**. Swagger UI(T6-3a)로 "직접 확인 가능한 형태" 일부 대체
