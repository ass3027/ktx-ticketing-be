...
- [ ] **T3-10** Redis-DB reconcile 잡: 잔여/활성자 카운터·`avail` 드리프트 주기 보정 → DB(SoT)로 수렴. **착수 전 `KTX_Ticketing_Reconcile_Design.md` 필독** — SADD in-flight 오버셀 함정·폐기된 해법(updatedAt/2회 스냅샷)·해법 두 갈래 정리됨. §10 열린 결정 2개(SADD 전략 기본·active 범위) 확정 후 구현.
