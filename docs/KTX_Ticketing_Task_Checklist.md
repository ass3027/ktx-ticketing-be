...
- [x] **T6-6** 셀프 체크리스트 7항목 점검 **완료**(2026-07-14). C1~C7 산출물 대조 → **6/7 충족(C1·C2·C4·C5·C6·C7), C3만 부분 미달**(배포 URL·동작 증거 대기 — T6-3b·T6-4/5 로 해소되는 산출물 공백이지 설계 결함 아님. T6-4/5 는 영상 대신 **스크린샷 세트**로 확보). 판정 근거는 아래 §제출 전 셀프 체크리스트에 항목별 기재. 변별력=C4(AI 게이트 워크플로우 실사례)·C7(E1 반직관적 결과 정직 서술)이 강점.
- [ ] **T6-7** 최종 점검 + 제출
...
> T6-6 점검(2026-07-14): **6/7 충족, C3만 부분 미달**(배포·영상 대기 — 설계 결함 아님, 산출물 완성으로 해소).
...
- [~] **C3** 배포 URL + GitHub + README + 동작 증거 구비 — GitHub·README ✅, **배포 URL(T6-3b)·동작 증거(T6-4/5) 미완**. 동작 증거는 영상 대신 **스크린샷 세트**(S1~S7: L1 oversell=0 정합성 캡처 + Swagger UI 실행)로 확보 예정 — 매체 선택이지 항목 공백 아님. Swagger UI(T6-3a)가 "직접 확인 가능한 형태" 실행 진입점
