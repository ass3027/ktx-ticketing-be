...
- [~] **T4-5** L3 조회 폭주 → 조회 경로 최적화(4기법 각 토글 측정). **Before**: 3,000TPS 목표에서 포화(list p95 초단위·dropped 대량), 병목=DB 커넥션 점유시간(SCARD 를 `@Transactional` 안에서 페이지 편수만큼 직렬 왕복). **① pool sweep**: 10→50 +14%만(한계효용 체감)→pool 은 지렛대 아님. **② SCARD tx 밖**(`ScheduleQueryReader` 분리·`booking.query.redis-outside-tx` 토글): 같은 세션 off↔on 에서 **usage_mean 6.9→2.6ms(−63%)·처리량 +62%·acquire −98%·pending 190→77** — 최대 단일 지렛대(근본=점유시간 확증). 단 여전히 포화·SLO(200ms) 미달 → **③ pipeline·④ 캐시(=E3 after) 진행 중**. 설계·측정: `docs/plans/Query_Path_Optimization_Plan.md`·`docs/results/P4_Result.md`(T4-5). 연관: T3-2 SCARD·T4-9 E3.
