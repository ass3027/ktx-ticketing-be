...
## T4-5 — L3 조회 폭주 (✅ 완료 · 조회 경로 최적화)

> 상태: **①~④ 전부 완료**(2026-07-07). Before 규명 + ① pool sweep + ② tx밖(07-04) + ③ pipeline 결론
> (N≈8 레버 아님, 07-06) + **④ 캐시=E3(07-07)**. ④는 단일 핫키(A4/C4)에서 포화 해소·SLO 통과, 다중 키
> (L3b)에서 캐시의 히트율 의존 한계까지 규명(jitter 부분 완화). **E3 Before/After 확보 → T4-9 E3 완료.**
...
### ② 누적 C2 — pool 을 tx밖 위에 얹으면? ①②는 상호 대체재 (pool50 off↔on·각 3회)

같은 세션에서 pool 을 50 으로 올려 C1(pool50·tx안=①만)과 C2(pool50·tx밖=①+②)를 측정, A2(pool10)와 교차 비교.
> 측정 인프라 주: C2 1차는 컨테이너 k6 가 원격 `jslib.k6.io/k6-summary` 를 매 run fetch 하다 Docker DNS
> flake(`no such host`)로 txon 전량 init 실패 → **jslib 로컬 vendoring**(외부 네트워크 무의존)으로 근본
> 제거 후 재측정. 이후 flake 0.

| 구성 | usage_mean | acquire_mean | pending | active | 실효 reqs/s | list p95 |
|------|------:|------:|------:|:---:|------:|------:|
| pool10·**tx안**(A2 base) | 6.9 ms | 133 ms | 190 | 10 | ~1,237 | 4.1 s |
| pool50·**tx안**(C1=①만) | 26.4 ms | 77.9 ms | 150 | 50 | ~1,604 | 3.0 s |
| pool10·**tx밖**(A2=②만) | 2.58 ms | 2.3 ms | 77 | 10 | ~2,000 | 2.35 s |
| pool50·**tx밖**(C2=①+②) | 5.13 ms | 0.73 ms | 62 | 50 | ~1,976 | 2.39 s |

- **핵심: ①②는 독립 지렛대가 아니라 상호 대체재.** tx밖(②)을 켠 상태에선 pool10(2,000)과 pool50(1,976)이
  **사실상 동일**(노이즈 내) — **② 위에 ①(pool)을 얹어도 처리량 이득 ≈ 0**. ② 가 이미 커넥션 점유(hold)를
  없애 pool 이 풀던 "커넥션 부족" 자체를 소거했기 때문. 둘은 같은 자원(커넥션 가용성)을 노린다.
- **pool 을 키우면 usage 가 오히려 오른다**(tx안 6.9→26.4ms, tx밖 2.58→5.13ms): 2코어 MySQL 에 커넥션
  50개가 몰리면 쿼리당 처리가 느려진다(DB 병렬 한계·경합). 처리량은 "느린 홀더 여러 개"로 조금 오를 뿐
  (C1 1,604), tx밖(hold 소거)이 준 도약(2,000)에는 못 미친다. **pool 은 증상 완화, ②는 원인 제거.**
- **C1 대비 C2(같은 sweep, 깨끗한 비교)**: pool50 에서 tx밖은 1,604→1,976(+23%)·usage 26.4→5.13ms(−81%)
  ·acquire 77.9→0.73ms. pool 을 아무리 키워도 tx밖 없이는 ~1,600 벽, tx밖이 그 벽을 뚫는다.
- **결론(①+②)**: **최종 도달점 ~2,000 TPS 는 tx밖(②) 하나로 이미 달성**되고 pool 은 기여하지 않는다 →
  운영 pool 은 굳이 올릴 필요 없음(기본 10 유지, DB 경합 회피). 남은 격차(2,000 vs 목표 3,000·p95 2.3s vs
  200ms)는 ③(pipeline)·④(캐시)가 SCARD 왕복 자체를 줄여야 메움.
- **주의(교차 sweep)**: pool10 행(A2)과 pool50 행(C1/C2)은 ~30분 간격 다른 sweep(같은 세션·동일 스택).
  clean 비교는 각 sweep 내부(A2 off↔on, C2 off↔on). 교차(pool10-on↔pool50-on)는 경향 판단용.

### ③ SCARD pipeline — 이 시스템의 운영점(N≈8)에선 레버가 아님 (효과크기 논증으로 측정 갈음)

SCARD 를 페이지 편수 N 만큼 직렬 왕복하던 것을 파이프라인 1회로 묶는다(RTT×N → RTT×1). 구현·테스트는
완료(`RedisSetPreemption.availableCounts`, 실 Redis 통합테스트 green). **다만 측정은 하지 않고 효과크기
논증으로 갈음**한다 — 이유는 아래.

| N (조회 페이지 크기) | pipeline 절감(RTT×(N−1)) | 포화 p95(~2.3s) 대비 | 판정 |
|------:|------:|:---:|:---:|
| **8 (실운영, `DEFAULT_LIMIT`)** | ~7 RTT ≈ **~1.4 ms** | 측정 바닥·회차 간 노이즈 이하 | **레버 아님** |
| 50 (시드 상한) | ~49 RTT ≈ ~10 ms | 겨우 가시 | **운영점 아님 → 무의미** |

- **효과가 N 에 비례한다**: pipeline 이 없애는 건 "명령당 RTT×(N−1)". 같은 docker 네트워크 Redis RTT ≈
  0.1~0.3ms 라, 운영 페이지 크기 N≈8 에선 총 절감이 **~1.4ms** — ②가 이미 tx 밖으로 뺀 SCARD 왕복
  구간에서, 그 왕복 횟수를 줄여도 절대량이 회차 간 분산 이하다.
- **N 을 부풀린 측정은 measurement theater**: 효과를 "보이게" 하려면 N=50 이 필요한데, 실운영은 N≈8±2 다.
  N=50 에서 이득이 나와도 "N=50 이면 도움"일 뿐 이 시스템엔 무관 — 존재하지 않는 운영점을 재는 것이라
  오히려 오해를 부른다. 그래서 **N 부풀리기 대신, 운영 N 에서 효과가 측정 바닥 이하임을 논증**하는 쪽을 택했다.
- **결론**: **이 시스템(N≈8)에서 ③ pipeline 은 조회 p95/포화점 레버가 아니다.** 단 코드는 되돌리지 않는다 —
  대용량 페이지 요청(limit 큰 경우)에 대한 정합성/안전 장치로 무해하고, N 이 커지면 자동으로 이득이 나는
  구조라 유지가 합리적. **남은 격차(2,000 vs 3,000 TPS·p95 2.3s vs 200ms)의 실질 레버는 ④(캐시=E3)** —
  SCARD 왕복을 "줄이는" ③이 아니라 "없애는" ④가 필요하다.
- **엔지니어링 판단으로서의 가치**: "N 의존 최적화라 우리 운영점에선 효과가 없음을 **알고** 채택 여부를
  판단했다"는 것 자체가 딜리버러블(효과크기 선(先)추정 → 무의미한 측정 회피). ②와 대비되는 음성(negative)
  근거로도 유효 — ②(점유시간)는 큰 레버, ③(왕복 횟수)는 이 스케일에선 아님.

### ④ 조회 단기 캐시 = E3 — DB 풀 병목을 우회해 포화 해소 (독립 A4, baseline ②③ off·각 3회)

리스트+잔여석을 Redis 공유 캐시에 짧은 TTL(1s)로 담아 히트 시 **DB 커넥션 미획득 + SCARD×N 소거**.
②③이 커넥션 점유시간·왕복을 *줄이는* 레버였다면, ④는 히트 경로에서 DB·SCARD 를 *아예 없앤다*.
stampede 는 기존 `DistributedLock` double-check single-flight 로 만료 순간 DB 재계산 1회를 보장한다.

| 회차 | cache | 실효 TPS | 조회 p95(list) | dropped_iterations | 판정 |
|:----:|:-----:|--------:|--------------:|-------------------:|:----:|
| run1 | off | 886 | 7.79s | 435,443 | 포화 |
| run2 | off | 814 | 9.34s | 454,970 | 포화 |
| run3 | off | 887 | 8.02s | 435,303 | 포화 |
| run1 | **on** | **2,497** | **28.07ms** | **0** | ✅ SLO |
| run2 | **on** | **2,497** | **22.93ms** | **0** | ✅ SLO |
| run3 | **on** | **2,497** | **26.92ms** | **0** | ✅ SLO |

- **cache off (= E3 Before, §1 L3 와 정합)**: ~850 TPS 포화. dropped ~44만 = 목표 3,000 TPS 미발사 →
  부하 모델 붕괴 상태의 p95(8~9s)라 절대치 아닌 "포화" 신호. (①②③ 어느 것도 여기서 SLO 를 못 넘겼다.)
- **cache on (= E3 After)**: **dropped=0 으로 목표 도착률을 실제 발사** — 즉 서버가 3,000 TPS 를 소화하며
  실효 2,497 TPS(목표에 근접), **조회 p95 ~26ms 로 SLO(200ms) 대비 ~7배 여유**. 3회 완전 일관.
- **왜 ④만 SLO 를 넘겼나**: T4-5 가 규명한 근본 병목은 **DB 풀 점유**다. ①(pool↑, +14%)·②(tx밖, +62%)는
  그 점유를 *완화*해 포화점을 밀었지만 여전히 풀을 거쳐 SLO 미달이었다. ④는 히트가 **tx 밖 GET 1왕복**이라
  풀을 아예 안 거친다 → 병목을 우회. 히트율이 높은 조회 폭주(L3)에서 포화가 사라진다.
- **정합성**: 예매 경로는 캐시 미접촉(강한 일관성 critical section 에서 재검증) → **오버셀 0 불변**. 표시
  staleness ≤ TTL(1s, 2-tier 계약 내). "매진인데 available" 이 ≤1s 뜰 수 있으나 예매 시도가 SREM=0 으로
  self-correct. 통합테스트로 캐시 on≡off 응답 등가 + round-trip 정합 검증 완료.

### ④ 누적 C4 — ②tx밖 위에 캐시: 캐시가 켜지면 ②는 잉여 (pool10·redisOutsideTx on·각 3회)

| 회차 | cache | 실효 TPS | 조회 p95(list) | dropped | 판정 |
|:----:|:-----:|--------:|--------------:|--------:|:----:|
| run1 | off | 960 | 6.67s | 415,444 | 포화 |
| run2 | off | 1,002 | 6.85s | 404,198 | 포화 |
| run3 | off | 1,031 | 6.64s | 396,317 | 포화 |
| run1 | **on** | **2,498** | **24.06ms** | 0 | ✅ SLO |
| run2 | **on** | **2,498** | **26.18ms** | 0 | ✅ SLO |
| run3 | on | 2,493 | 31.92ms | 1,935 | ✅ SLO |

- **cache off (②tx밖만)**: ~1,000 TPS 포화 — A4 baseline(~850)보다 ②만큼 높지만(§ ② 로그 +62% 정합)
  여전히 SLO 미달. cache on 은 **A4(2,497)와 사실상 동일**(2,498).
- **결정타 — 캐시가 켜지면 ②③은 잉여**: 히트 경로는 애초에 DB·SCARD 를 안 타므로 ②(tx밖 SCARD)의
  이점이 무의미해진다. **A4≈C4** 는 "②가 ①을 잉여로 만든다"(§ C2 로그)의 연장 — ④는 ①②③까지 잉여로
  만드는 **상위 레버**다. 운영 조합은 캐시 on + pool10 + ②③ 기본값으로 족하다(캐시가 다 흡수).

### ④ 한계 — 다중 키(낮은 히트율)에선 캐시만으로 SLO 미보장 (L3b, from 50개 분산)

> A4/C4 는 **단일 노선·단일 from = 캐시 키 1개(핫키) → 히트율 ~100%** 의 상한 효과다. 실제는 여러 키가
> 골고루 요청될 수 있으므로, `from` 을 시드된 50일에 균등 분산해(키 50개, 히트율 ~98%) 낮은 히트율을 실측했다.

**Before(고정 TTL) — cache on 5회**:

| 지표 | run 별 범위 | 관찰 |
|------|:---:|------|
| 조회 p95(list) | 31 ~ **525ms** | 회차마다 15배 이상 요동(단일 키는 22~32ms 로 타이트) |
| max(list) | 0.7 ~ 1.86s | **매 회 초 단위 꼬리** — 만료 순간 DB 파도 |
| dropped | 0 ~ 6,560 | 스파이크로 순간 도착률 미발사 |

- **원인 = 다중 키 동시 만료 스파이크**: 부하 초반 50개 키가 거의 동시에 채워져 → 고정 TTL(1s) 이라
  거의 동시에 만료 → 만료 순간 50개 미스가 뭉쳐 DB(풀10)에 파도. single-flight 는 키별 재계산을 1회로
  막지만 **키가 50개라 동시 50개 컴퓨트**가 풀을 순간 점유 → p95 꼬리가 튄다. **5회 재현**(일회성 아님).

**After(TTL jitter 0.2 = ±20%) — cache on 3회**:

| 지표 | Before(고정) | After(jitter 0.2) | 효과 |
|------|:---:|:---:|:---:|
| 최악 p95 | 525ms | **265ms** | 절반 ↓ (그러나 SLO 200ms 초과 유지) |
| max | 0.7~1.86s | 0.95~1.57s | 소폭 ↓ |
| dropped | 0~6,560 | 133~3,227 | 소폭 ↓ |

- **jitter 는 부분 완화에 그침(정직한 결론)**: TTL 을 ±20% 흔들어 만료를 시간축에 분산 → 최악 p95 를 절반
  으로 줄였으나 **SLO(200ms)는 여전히 못 넘긴다**. 이유: jitter 는 "만료가 **동시에** 뭉치는 것"만 흩뜨릴
  뿐, **미스 총량(키 50개 × 만료율)은 그대로**다. 다중 키는 초당 미스 자체가 많아, 만료를 펴도 미스 유입이
  주기적으로 DB 풀 처리량을 초과한다 — 파도를 잔물결로 바꿀 뿐 물의 양은 같다.
- **결론(캐시의 적용 범위)**: **캐시는 히트율 높은 핫키(인기 노선·시간대)에 강하고**(A4/C4: 2,497 TPS·
  p95 26ms), **낮은 히트율에선 DB 풀이 병목으로 복귀**해 캐시만으로 SLO 를 보장 못 한다. 실 KTX 도 조회는
  소수 핫키에 집중되므로 A4 가 대표 시나리오이나, **"캐시 = 만능"이 아니라 "히트율 의존"** 임을 다중 키로
  정직하게 규명했다. 낮은 히트율 대응은 별도 레버(미스 경로 ②tx밖 결합, DB 풀 상향, 조회 결과 DB 인덱싱)가
  필요하며, 이는 이 시스템 운영점(핫키 조회) 밖이라 후속 과제로 남긴다.
- **jitter 는 무해한 상시 옵션으로 유지**: 최악값을 절반으로 낮추는 실익이 있고 고정 TTL 대비 부작용이 없다
  (staleness 상한만 ttl×1.2 로 소폭 증가, 2-tier 계약 내). 기본 0(off), env `BOOKING_QUERY_CACHE_TTL_JITTER`
  로 토글.
