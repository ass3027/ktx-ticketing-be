...
| T4-5 L3 조회 폭주 | ⏳(측정중) | **Before 규명 + ①②(독립 A2·누적 C2) 완료 + ③ 결론**: 3,000TPS 목표에서 포화. 병목=커넥션 점유시간. **① pool: 지렛대 아님**. **② SCARD tx밖: usage 6.9→2.6ms(−63%)·처리량 +62%** — 최대 단일 지렛대. **누적: ②위에 ①얹어도 이득≈0(①②는 상호 대체재)**. **③ pipeline: N≈8 운영점에선 ~1.4ms=노이즈 이하 → 레버 아님(효과크기 논증, 측정 갈음)**. 단 여전히 SLO 미달(~2,000/p95 2.3s) → ④(캐시=E3) 진행 |
...
> 상태: **Before 규명 + ① pool sweep + ② tx밖 완료**(② 2026-07-04). **③(pipeline) 결론 기록**(N≈8 에선
> 레버 아님 — 효과크기 논증으로 측정 갈음, 2026-07-06). ④(캐시=E3) 진행.
