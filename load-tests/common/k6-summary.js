// Vendored: k6-summary v0.0.1 (https://jslib.k6.io/k6-summary/0.0.1/index.js).
// 부하 테스트를 외부 네트워크에 의존시키지 않으려 로컬 고정 — 컨테이너 k6 가 매 run 마다
// 원격 fetch 하다 DNS flake(no such host)로 측정이 깨지는 문제를 근본 제거(T4-5 ② C2 계기).
// 업스트림 원본 그대로(수정 금지). textSummary/humanizeValue/jUnit export.
