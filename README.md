...
> 7주 단계별 계획(P0~P6)으로 설계·구현. 부하 시나리오 **L1~L6**(9종) + 필수 실험 **E1~E3** Before/After, 테스트 28개 클래스(통합 11 + 단위/슬라이스). 스택: Spring Boot 4.0 / Java 25 · MySQL 8 · Redis 7 · Redisson · k6.

---

## 왜 이 문제인가

명절마다 KTX 예매창에서 새로고침을 반복하다 좌석을 놓치곤 했다. 조회는 느렸고, 남은 좌석 수가 화면마다 달라 보이는 정보 불일치도 겪었다. KTX 예매는 일상에서 가장 자주 접하는 대규모 트래픽 서비스라, 그 이면의 동시성·정합성 문제를 직접 파고들 소재로 선택했다.
*같은 좌석을 동시에 노린 여러 요청 중 누구를 성공시키고 나머지를 어떻게 정확히 거절할 것인가* — 이 프로젝트는 그 문제를 다음과 같이 다룬다:
...
KTX 예매의 본질 난제는 "같은 좌석 동시 점유 제어"이므로, **분산 락으로 경합을 직접 제어**하는 방식이 학습·어필 가치가 가장 크다. 실제 KTX 앱처럼 **순번을 노출하지 않는 입장 제어**가 도메인에도 충실하다. (상세: [docs/KTX_Ticketing_Architecture_and_Verification_Goals.md](docs/KTX_Ticketing_Architecture_and_Verification_Goals.md))
...
- **조회 리스트 캐시 (single-flight)** — 운행 리스트/잔여석 조회는 Redis 공유 단기 캐시(TTL 1s)로 서빙해 히트 시 DB 커넥션·`SCARD` 집계를 소거한다. 핫키 TTL 만료 순간 요청이 몰려 일제히 DB로 쏟아지는 **스탬피드**를 **single-flight**(winner 1개만 재계산·나머지는 결과 공유)로 막는다. 능동 무효화는 두지 않고 **staleness를 TTL(≤1s)로 제한**, 낡은 표시는 예매 시도가 `SREM=0`으로 자가교정한다(2-tier 계약). E3에서 핫키 p95 8s→26ms의 실체.
- **Redis↔DB 드리프트 수렴 (DB = SoT)** — 부하 중 벌어진 Redis `avail` Set ↔ DB 상태의 드리프트를 `ReconciliationService`가 주기적으로(60s) **DB(진실의 원천)로 수렴**시킨다. 방향은 비대칭 — DB엔 없는데 Redis에만 남은(stale) 좌석 제거는 상시 안전하고, 그 반대(missing) 복원은 선점~커밋 사이 in-flight 오탐을 피해 grace(5분) 경과분만 되돌린다 → **오버셀을 원천 차단**. 이로써 위 2단 일관성 모델의 "Redis=빠른 게이트, DB=SoT"가 실제로 닫힌다(빠른 게이트는 언제든 틀릴 수 있고, DB가 최종 정정한다).
...
> **이중 방어선**: ① Redis `SREM` 원자 선점이 패배 999건을 DB 앞단에서 즉시 반려(부하 흡수), ② DB `@Version` 낙관락 + `uk_active_seat`가 최종 정합성 방어. 이 이중 구조가 만든 반직관적 결과 — *선점을 꺼도 오버셀은 0* — 를 E1 실험이 수치로 규명한다(아래).
...
목표를 "빠르게·안정적으로" 같은 애매한 말로 두지 않고 **측정 가능한 수치(SLO)로 못박아** 달성 여부를 판정한다.
**우선순위는 정합성(S4)이 최우선** — 예매 시스템의 절대 조건이라 하나라도 깨지면 실패다. 그다음이 응답 성능(S1·S2)·
처리량(S3)·가용성(S5)이며, S6(임계점)는 "어디서 무너지는가"를 수치로 아는 운영 목표다. 각 기준값은 업계 레퍼런스
(Google SRE 등)를 참조해 직접 정의한 뒤 실측으로 검증했다 — 핵심은 숫자 크기가 아니라 **"왜 이 값인가"를 설명할 수 있는 것**이다.
...
> 실험 E1·E2·E3 의 Before/After 짝이 모두 완료**됐다.
...
- **2-tier 일관성의 명시적 분리** — 조회/표시는 약한 일관성(빠름), 예매 확정은 강한 일관성(정확). 둘을 섞지 않고
  ...
  처음 예매창에서 겪은 *"남은 좌석 수가 화면마다 달라 보이는" 불일치*가 바로 이 약한 일관성의 표면이다 — 조회를
  DB 강한 일관성으로 묶으면 매 요청이 재고를 집계하느라 조회 p95 가 초 단위로 무너진다(E3 Before: 캐시 off
  p95 8~9s). 그 불일치는 **버그가 아니라 조회 속도를 위해 의도적으로 감수한 트레이드오프**이며, 정확성이 필요한
  예매 확정만 강한 일관성으로 격리했다. 이 프로젝트는 그 경계를 어림짐작이 아니라 SLO 수치(S2b staleness ≤ 2s ·
  S4 oversell 0)로 못박아, "어디까지 흐릿해도 되고 어디부터 정확해야 하는가"를 측정 가능한 설계 결정으로 만들었다.
...
| 동작 증거 — 정상 흐름 | ✅ 예매 happy path **시퀀스 다이어그램**(과정 가시화) + **Swagger UI 라이브 데모**(`/swagger-ui.html` 직접 호출) + 통합 테스트 E2E(`BookingIntegrationTest`) | T6-3a/T6-4 |
