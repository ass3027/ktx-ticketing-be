...
> 측정값은 [docs/results/P4_Result.md](docs/results/P4_Result.md)의 실측 인용. **L3(조회 폭주)·L5(임계점)·
> L6(soak)·E3(캐시)는 완료**됐고, 남은 빈 칸은 E1·E2(선점/입장 제어 Before)뿐이다. 기준값은 1차 측정 후
...
> E1·E2·E3는 필수. 각 실험은 그래프 + 한 줄 해석으로 정리한다. **E3는 완료**(위 수치), 남은 건 E1·E2의
> Before(선점/입장 제어 off) 측정 — 정규 부하(L1/L4)에서 After 신호는 이미 확보돼 있어 T4-9에서 토글 반대쪽만
> 측정해 짝을 완성한다. 추가 실험(E5 가상 스레드·E6 분산 락 라이브러리·E7 선점 백엔드)은
...
|------|------|-----------|
| GitHub 소스코드 | ✅ (이 레포) | — |
| README (문제정의·아키텍처·트레이드오프·성능) | 🟡 골격+L3/L5/L6/E3 수치 반영, E1·E2 채우는 중 | T4-9 (E1·E2) |
